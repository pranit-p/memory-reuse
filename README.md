...
| `langchain` | Faithful LangChain message (de)serialization for `wrap_graph` | `pip install memory-reuse[langchain]` | `uv add memory-reuse[langchain]` |
...
Three opt-in helpers that make the cache smarter and safer. All are additive —
leave them at their defaults and the cache behaves exactly as before (including
byte-identical keys).

### Single-flight — run expensive work once under a burst

When many concurrent requests miss the same key at once, `get_or_compute` runs
the compute **once** and shares the one result, instead of every caller paying:
...
For multiple processes / microVMs sharing one Redis backend, add
`distributed_lock=True` to coalesce across processes too (it's fail-open — a lock
fault never blocks or raises, it just computes):
...
### Version-aware cache identity — never serve stale results after a redeploy

Fold a version token into every key so a new deployment (new prompt, model, or
tool) never reuses a result generated by the old logic:

```python
cache = MemoryCache(CacheConfig(cache_version="agent-v2"))
```

Unset (the default), keys are byte-identical to before, so existing cached
entries keep hitting. A version change invalidates reuse across the exact, tool,
and semantic layers at once.

### Effectiveness analyzer — what to cache, and what not to

Record per-operation hit/miss observations, then ask for **actionable
recommendations**:
...
Recommendations are **advisory only** — reading them never changes cache
behaviour. The `cached` and `volatile` flags are yours to supply, because the
analyzer can't infer a tool's caching status or data-freshness on its own.
...
| 6 | Intelligent optimization: single-flight, version-aware identity, effectiveness analyzer + recommendations | ✅ Shipped |
